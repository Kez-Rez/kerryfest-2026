...
async function downloadTicket() {
    ...
    try {
        // Give more time for all images and barcodes to fully render
        await new Promise(resolve => setTimeout(resolve, 1000));

        console.log('Starting download of', tickets.length, 'ticket(s)');
        ...
        for (let i = 0; i < tickets.length; i++) {
            ...
            const ticketNumber = tickets.length > 1 ? `-${i + 1}` : '';
            link.download = `kerrytix-birthday-ticket${ticketNumber}.png`;
            ...
            console.log('Download triggered for ticket', i + 1);

            // Small delay between downloads if multiple tickets
        ...
        alert('Unable to download ticket. Error: ' + error.message + '\n\nPlease check the browser console for details, or use the Print function instead.');
    }
}
